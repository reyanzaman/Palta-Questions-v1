...
import { findGeneral } from "../helper/helper";
...
export default function GeneralQuestions() {
	...
	const questionSort = (questions) => {
		const sortedIndices = Array.from(questions.keys()).sort((a, b) => questions[b]["q1Score"] - questions[a]["q1Score"]);
	  
		const sortedQuestions = sortedIndices.map((index) => {
		  const question = questions[index]["question1"];
		  const { semester, year, section, date, isAnonymous, username } = questions[index];
		  const score = questions[index]["q1Score"];
